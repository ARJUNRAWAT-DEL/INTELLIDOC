...
const StunningUpload: React.FC<StunningUploadProps> = ({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Backend may return task id under different keys depending on version
      const taskId = (response as any).task_id || (response as any).upload_task_id || (response as any).uploadTaskId || (response as any).taskId;
      console.log("Upload response:", response, "resolved taskId:", taskId);

      if (taskId) {
        onUploadStart?.(taskId);
      } else if ((response as any).status === "completed" || (response as any).result) {
        // Backend processed synchronously and returned result immediately
        console.log("Upload completed immediately, calling onUploadComplete with result:", (response as any).result || response);
        onUploadStart?.(null as any);
        onUploadError?.(undefined as any);
        // call complete callback if provided
        // prefer result object if present
        onUploadStart?.((response as any).task_id as any);
        (onUploadError as any) || null;
        // invoke onUploadComplete if parent provided it
        try {
          onUploadComplete?.((response as any).result || response);
        } catch (e) {
          // ignore if prop not present
        }
      } else {
        // No task id and no immediate result — still notify parent so it can handle
        onUploadStart?.(null as any);
      }
    ...
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-lg text-gray-600 mb-8 max-w-md mx-auto">
          Upload any document and unlock its potential with AI-powered search
          and analysis
        </p>
      </div>
          ...
          <div className="pointer-events-none relative z-10">
            {!file ? (
              <>
                {/* Upload Icon */}
                <div
                  className={`w-24 h-24 mx-auto mb-8 rounded-3xl flex items-center justify-center transition-all duration-500 ${isDragging ? "bg-emerald-100 scale-105 shadow-md" : "bg-gray-50 shadow-sm"}`}
                >
                  <svg
                ...
                <div className="flex items-center justify-center space-x-6">
                  {[
                    ...
                    <div
                      key={format.ext}
                      className={`flex items-center space-x-2 bg-white px-4 py-2 rounded-full border border-gray-100 shadow-sm transition-all duration-200 ${isDragging ? "scale-105" : ""}`}
              ...
              <>
                ...
                <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-sm max-w-md mx-auto">
                  <div className="flex items-start space-x-6">
                    <div
                      className={`w-16 h-16 bg-gradient-to-br ${getFileColor(file.name)} rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg`}
                    >
                      <span className="text-2xl">{getFileIcon(file.name)}</span>
                    </div>
                    <div className="flex-1 text-left">
                      <h4 className="font-bold text-gray-900 mb-2 text-lg break-words">
                        {file.name}
                      </h4>
                ...
                <div className="mt-8 text-center">
                  ...
                  <p className="text-gray-600 text-sm">
                    Your document will be analyzed and made searchable
                  </p>
                </div>
              </>
            )}
      ...
      {file && (
        <div className="text-center">
          ...
          <p className="text-gray-500 text-sm mt-4">
            Your document will be processed with AI and made instantly
            searchable
          </p>
        </div>
      )}
