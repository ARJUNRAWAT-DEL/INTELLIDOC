...
export default function BookDemoModal({ isOpen, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div
        ref={modalRef}
        className="relative max-w-lg w-full bg-white rounded-2xl shadow-lg p-6 z-10"
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold">Book a demo</h2>
            <p className="text-sm text-gray-600 mt-1">
              Tell us a few details and we’ll send an invite.
            </p>
          </div>
          <button
            aria-label="Close"
        ...
        {!submitted ? (
          <form onSubmit={onSubmit} className="mt-4 space-y-4">
            ...
            <div className="flex items-center justify-between gap-3">
              <button
                type="submit"
                className="px-4 py-2 rounded-full bg-[var(--accent-pink)] text-white font-semibold"
              >
                Request demo
              </button>
              <button
          ...
          <div className="mt-6 text-center">
            ...
            <div className="text-sm text-gray-600 mt-2">
              We’ll email you a calendar invite shortly.
            </div>
            <div className="mt-4">
              <button
                onClick={onClose}
                className="px-4 py-2 rounded-full bg-[var(--brand-900)] text-white"
              >
                Done
              </button>
            </div>
